...
main(int argc, char *argv[])
{
  ...
  if (!initiator_only_mode) {
    ...
    ntEmulatedTarget.nti.nai.abtAts[1] = 0x00;
  ...
  label:
      ...
      if (ret) {
        ...
        if (waiting_time != 0 || waiting_time2 !=0) {
          ...
          if(microsecondes)
          {
        	/*int *r = pthread_create (&ta, NULL, task_a,(time_t) (waiting_time2*1000000));
        	if(*r == -1)
        	{
        		nfc_exit(context);
        		exit(EXIT_FAILURE);
          ...
          if(secondes){
        	/*int *r = pthread_create (&ta, NULL, task_a, (time_t) (waiting_time));
        	if(*r == -1)
        	{
        	   nfc_exit(context);
        	   exit(EXIT_FAILURE);
